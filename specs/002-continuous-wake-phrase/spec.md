...
- **FR-008**: The system MUST excise the wake phrase from wherever it falls and submit all remaining buffered speech as one prompt, including speech that followed the phrase in the same utterance. The phrase introduces an instruction as often as it follows one, so holding back what came after it would split a single spoken instruction across two submissions. The phrase itself MUST be excluded from what is sent, and the buffer MUST be left empty.
