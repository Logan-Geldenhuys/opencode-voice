...
- **FR-008**: The system MUST submit the entire buffer as one prompt, wake phrase included and in the position it was spoken, and MUST leave the buffer empty. The phrase introduces an instruction as often as it follows one, so nothing may be held back on either side of it. The phrase MUST NOT be removed: it is how the developer addresses the assistant directly, and it is therefore the only marker distinguishing the instruction from the thinking-aloud around it. The system MUST NOT submit when the buffer holds nothing but the phrase.
- **FR-009**: The system MUST label submitted text as a voice transcript that may contain recognition errors, particularly in code identifiers and file paths, so the agent can weigh unfamiliar terms accordingly. The label MUST also tell the agent the name the developer addresses it by, and that speech marked with that name is a direct instruction while the surrounding speech is context. Retaining the phrase under FR-008 only helps if the recipient knows what it signifies.
...
- **FR-014**: The system MUST bound the buffer both by a configurable maximum age and by a configurable maximum size, discarding the oldest speech first when either bound is exceeded, so that speech from much earlier cannot be submitted to the agent by a later wake phrase. Eviction MUST remove whole earlier utterances rather than truncating text partway through one, so that every utterance surviving in the buffer is the text that was actually spoken. The two bounds are set to describe the same envelope from different directions: the size bound is what fires when the developer has genuinely been talking, and the age bound is what fires when they have not.
...
- **SC-002**: Given text containing any configured accepted form of a wake phrase — including forms differing only in spacing, punctuation or capitalisation, and forms spanning a pause boundary — the correct action is resolved in 100% of cases. This criterion is evaluated exhaustively against text rather than sampled from audio, which is why the target is total rather than statistical: how reliably a phrase is _recognised_ is a property of the transcription service and is addressed by SC-012, whereas how reliably a recognised phrase is _acted on_ is a property of this feature and admits no tolerance.
- **SC-003**: The interrupt variant is never resolved as the plain variant: zero misclassifications across every configured accepted form of both phrases.
...
- **SC-005**: Silence and sounds shorter than the configured minimum duration generate zero transcription requests, verified by driving the segment-handling path with captures on both sides of the threshold and counting the requests issued.
...
- **SC-009**: When either buffer bound is exceeded, the speech discarded is the oldest and the speech retained is undamaged: no submitted prompt contains an utterance captured longer ago than the age bound, and no submitted prompt contains a partial utterance. The bound that will fire in practice is the size bound; the age bound is a backstop for a session left open without speech, so this criterion is not on its own the feature's protection against stale speech being submitted — inspecting and discarding the buffer is.
...
- **SC-012**: After the one-time calibration of accepted phrase forms against the developer's own voice, microphone and room, speaking each wake phrase ten times triggers the intended action at least nine times. This is a calibration acceptance check performed by the developer, not an automated gate: its input is live speech and its output is configuration.
...
- The developer is willing to perform two one-time calibrations for their microphone and room: pause-detection sensitivity, and the accepted forms of each wake phrase as their own voice is actually transcribed. Neither can be established by design, because both depend on the room, the microphone and the speaker. Both produce configuration rather than code, which is why neither is automated and neither is a continuous-integration gate.
