...
export const DEFAULT_WAKE_PHRASES = [
  ...
  // accepted, truncating the prompt at a phrase the speaker never said. They
  // are therefore absent, and the remaining forms keep the phrase rare.
  //
  // Being two tokens, this phrase has less margin than the others. If
  // calibration turns up a form that is also ordinary speech, prefer lengthening
  // the phrase over accepting the form.
  //
  // Calibration produced exactly that case: unbiased, the recogniser returns
  // "hey node" -- and "node" is the most ordinary word a JavaScript developer
  // says. Accepting it fires on 5 of 9 plausible utterances ("hey node is
  // crashing on startup", "I said hey node and it ignored me"), so it is not a
  // variant. The fix belongs upstream of matching: adding "Nome" to
  // sttVocabulary biases the recogniser and it returns "Hey Nome" in
  // isolation, mid-sentence and sentence-initial. A phrase that is not a word
  // needs to be taught to the recogniser, not guessed at afterwards.
  {
    canonical: "hey nome",
