...
export const DEFAULT_WAKE_PHRASES = [
  ...
  // A shorter phrase to say, and the variants are chosen rather than
  // enumerated. "nome" has homophones that are ordinary English words, and two
  // of them are load-bearing in speech about code: "hey name the function
  // fetchUser" and "hey known issue" both match if `name` and `known` are
  // accepted, firing at a phrase the speaker never said. They are therefore
  // absent, and the remaining forms keep the phrase rare.
  //
  // "hey node" is the exception, and it is accepted deliberately. It is what
  // the recogniser actually returns for this speaker -- 18 of 18 utterances,
  // and adding "Nome" to sttVocabulary did not shift it -- so excluding it
  // makes the phrase unusable rather than safe. It costs the most false
  // positives of any form considered (5 of 16 plausible utterances mentioning
  // node), and that is the accepted price of a phrase that fires at all.
  //
  // Two things make the price bearable. A false positive now submits the whole
  // buffer rather than a prefix of it, so nothing is silently truncated: the
  // developer sees their own words and says the phrase again. And the
  // transcript label warns the agent that "hey node" here is this phrase
  // mis-transcribed rather than a reference to Node.js, so the retained form
  // does not read as a topic.
  //
  // Being two tokens, this phrase has less margin than the others. For any
  // form other than the one the recogniser insists on, prefer lengthening the
  // phrase over accepting the form.
  {
    canonical: "hey nome",
    variants: ["hey node", "hey gnome", "hey nom", "hey no me"],
    action: SUBMIT,
  },
];
