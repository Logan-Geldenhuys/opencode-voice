...
// Enough silence to give the recogniser an onset boundary, and deliberately not
// configurable: it restores a fixed 100ms detection window the recorder's own
// silence clause discards, so there is nothing here for a developer to tune
// against their room. 300ms was measured; the failure it fixes is binary rather
// than gradual, so a margin costs a third of a second of upload and buys
// certainty. See padLeadingSilence.
const ONSET_PAD_MS = 300;
...
// Compose the transcription `prompt` field from a style instruction and the
// vocabulary terms.
//
// Both land in the same parameter because the service offers only one. The
// instruction goes first: it governs how the whole utterance is rendered, while
// the terms only bias individual words, and a trailing word list reads as
// context rather than as a competing directive.
//
// Whether the instruction is honoured is a property of the model, not of this
// function. Measured on this gateway: `whisper-1` obeys it, and `gpt-transcribe`
// ignores it entirely - the same prompt that makes whisper-1 render the spoken
// address correctly in every position leaves gpt-transcribe producing "gnome"
// mid-sentence across four identical runs. An instruction is therefore worth
// sending, but not worth trusting a model to follow without measuring it.
export function buildTranscriptionPrompt(instruction, terms) {
  const prefix = typeof instruction === "string" ? instruction.trim() : "";
  const vocabulary = buildVocabularyPrompt(terms);
  if (!prefix) return vocabulary;
  if (!vocabulary) return prefix;
  return `${prefix}\n${vocabulary}`;
}

// Restores the speech onset that silence trimming discards.
//
// Both capture modes start their recorder with a leading `silence 1 0.1 <thr>`
// clause, which is what makes the recorder wait for speech rather than record
// the pause before it. That clause does not merely skip the pause: it discards
// everything before the threshold crossing, so the capture begins part-way
// through the first sound rather than just before it.
//
// Recognition needs that moment. Measured on a capture of "opencode execute"
// taken through the real recorder: as recorded it transcribes as the single
// token "OpenCodeExecute", which the wake matcher cannot match, because a
// phrase compiled from two words can never match one word. Padding the same
// bytes with 300ms of silence in front returns "Open code execute." and matches.
// Padding the end instead changes nothing at all, which is what says the onset
// is the missing information rather than the clip simply being short.
//
// The leading clause cannot be dropped to avoid this. It is what holds the
// recorder open until someone speaks; without it the stop clause fires during
// the pre-speech silence and every capture ends empty. So the information is
// destroyed where it has to be destroyed, and restored here, which is the one
// place both modes pass through.
export function padLeadingSilence(audioBuffer, milliseconds) {
  if (!Buffer.isBuffer(audioBuffer) || !(milliseconds > 0)) return audioBuffer;
  if (audioBuffer.length <= WAV_HEADER_BYTES) return audioBuffer;

  // Anything other than the canonical header sox writes is returned untouched.
  // Guessing at a layout would corrupt the upload, and an unpadded request
  // still transcribes - it is only less likely to preserve word boundaries.
  if (audioBuffer.toString("latin1", 0, 4) !== "RIFF") return audioBuffer;
  if (audioBuffer.toString("latin1", 8, 12) !== "WAVE") return audioBuffer;
  if (audioBuffer.toString("latin1", 12, 16) !== "fmt ") return audioBuffer;
  if (audioBuffer.readUInt32LE(16) !== 16) return audioBuffer;
  if (audioBuffer.readUInt16LE(20) !== 1) return audioBuffer;
  if (audioBuffer.toString("latin1", 36, 40) !== "data") return audioBuffer;

  const byteRate = audioBuffer.readUInt32LE(28);
  const blockAlign = audioBuffer.readUInt16LE(32);
  if (!byteRate || !blockAlign) return audioBuffer;

  // Whole frames only. A partial frame would shift every following sample by a
  // byte and turn the recording into noise.
  const frames = Math.round((byteRate * milliseconds) / 1000 / blockAlign);
  if (frames < 1) return audioBuffer;
  const silenceBytes = frames * blockAlign;

  const header = Buffer.from(audioBuffer.subarray(0, WAV_HEADER_BYTES));
  const silence = Buffer.alloc(silenceBytes);
  const samples = audioBuffer.subarray(WAV_HEADER_BYTES);

  // PCM silence is zero, so the padding needs no encoding - only the two length
  // fields have to agree with it, or the service reads the declared length and
  // ignores the rest.
  header.writeUInt32LE(audioBuffer.readUInt32LE(4) + silenceBytes, 4);
  header.writeUInt32LE(audioBuffer.readUInt32LE(40) + silenceBytes, 40);

  return Buffer.concat([header, silence, samples]);
}

export function buildMultipartTranscriptionRequest(model, audioBuffer, apiKey, prompt = "") {
...
export async function transcribeApi(kv, wavFile, logger, signal) {
  ...
  try {
    const audioBuffer = padLeadingSilence(await fs.promises.readFile(wavFile), ONSET_PAD_MS);
    const credential = resolveApiCredential();
    ...
    // parameter, so both the style instruction and vocabulary biasing apply to
    // the OpenAI-compatible path only. Sending an unrecognised field there
    // would be a request the service is entitled to reject.
    const prompt = buildTranscriptionPrompt(sttApiInstruction, sttVocabulary);
    const request = useOpenRouterFormat
      ? buildOpenRouterTranscriptionRequest(model, audioBuffer, apiKey)
      : buildMultipartTranscriptionRequest(model, audioBuffer, apiKey, prompt);
    if (prompt && !useOpenRouterFormat) {
      logger?.log("STT", `Prompting recognition with ${prompt.length} chars`, "debug");
    }
...
export function registerSTT(api, kv, complete, prompts, opts, logger, credentialResolver) {
  ...
  if (typeof opts?.sttApiInstruction === "string") {
    sttApiInstruction = opts.sttApiInstruction.trim();
    if (sttApiInstruction) {
      logger?.log("STT", `Recognition instruction=${sttApiInstruction.length} chars`, "debug");
    }
  }
