...
// transcribed in place of the current one.
...
// accounts on the host (FR-014).
function ensureCaptureDir(logger) {
  ...
  logger?.log("STT", `Capture directory=${captureDir}`, "debug");
  return captureDir;
}

// Called when the editor disposes the plugin, so the directory does not
// accumulate across sessions (FR-018).
export function removeCaptureDir(logger) {
  ...
  try {
    ...
    logger?.log("STT", `Removed capture directory ${dir}`, "debug");
  } catch (err) {
    logger?.log("STT", `Failed to remove capture directory ${dir}: ${err.message}`, "warn");
  }
}
...
export function createCapture({ process: child, path: audioPath, logger }) {
  ...
  const cap = {
    ...
    async stop() {
      ...
      if (info) return info;
      logger?.log("STT", `Capture pid=${child.pid} ignored SIGINT`, "warn");
    ...
    // process the same user happens to be running: exactly the behaviour
    // FR-016 forbids.
    async terminate() {
      let info = await signalAndWait("SIGTERM", TERMINATE_GRACE_MS);
      if (!info) {
        logger?.log("STT", `Capture pid=${child.pid} ignored SIGTERM, sending SIGKILL`, "warn");
        info = await signalAndWait("SIGKILL", TERMINATE_GRACE_MS);
      }
      if (!info) logger?.log("STT", `Capture pid=${child.pid} survived SIGKILL`, "error");
    ...
    removeAudio() {
      try {
        ...
        logger?.log("STT", `Removed capture audio ${cap.path}`, "debug");
      } catch (err) {
        if (err.code !== "ENOENT") {
          logger?.log("STT", `Failed to remove capture audio ${cap.path}: ${err.message}`, "warn");
        }
      }
    },
  };
...
// Stop and clean up whatever is active. Safe to call when nothing is.
export async function stopActiveCapture(logger) {
  const active = capture;
  if (!active) return;
  capture = null;
  logger?.log("STT", `Stopping active capture pid=${active.process.pid}`, "debug");
  await active.terminate();
  active.removeAudio();
}

// Synchronous counterpart of the two functions above, for the process "exit"
// hook. By then the event loop is closing, so a promise will never settle and
// only blocking calls still run: kill outright and unlink in place.
...
function startRecording(kv, backend, toast, logger, trimSilence = true) {
  // At most one capture is active. A second trigger is refused by name rather
  // than starting a competing recorder that would fight for the microphone and
  // leave two files behind (FR-017).
  if (capture) {
  ...
  let audioPath;
  try {
    audioPath = buildCapturePath(ensureCaptureDir(logger), ++captureSequence);
  ...
  const active = createCapture({ process: child, path: audioPath, logger });
  capture = active;
  ...
  child.on("exit", (code) => {
    ...
    if (capture === active && active.state === "recording" && code !== 0 && code !== null) {
      capture = null;
      active.removeAudio();
...
async function getApiModels(logger) {
  ...
  try {
    ...
    if (credential.ok) {
      ...
      // Logged rather than raised: the caller renders a tier list, and an empty
      // list with a stated reason is more useful than a thrown error here.
      logger?.log(
        "STT",
        `Model catalogue request unauthenticated: ${formatCredentialError(credential.attempts).replace(/\n\s*/g, " ")}`,
        "warn",
      );
    }
...
async function transcribeApi(kv, wavFile, logger) {
...
async function doTranscribePipeline(
  ...
  try {
    ...
    // Nothing was captured. Not an error state, and deliberately not a
    // transcription request or a prompt insertion either.
    if (!result.text) {
