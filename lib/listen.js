...
export function createListener({
  ...
  async function submit(hit) {
    ...
    try {
      ...
      // the buffer already is the retained tail and `prompt` is a value that no
      // longer aliases it. The window a lock would have guarded does not exist
      // (research.md R-109).
      const now = Date.now();
      ...
      const spoken = found.before.trim();
      const tail = found.after.trim();

      entries = tail ? [{ text: tail, capturedAt: now }] : [];
