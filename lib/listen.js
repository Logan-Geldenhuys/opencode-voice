...
import { audioFailureSuffix, buildRecordArgs, detectAudioBackend, transcribeApi } from "./stt.js";
...
export function createListener({
  ...
  async function submit(hit) {
    ...
    try {
      ...
      // The phrase is excised from wherever it falls rather than treated as a
      // terminator, so "hey nome, fix the failing test" sends the instruction
      // it introduces. Detection runs after every append, so the phrase is
      // always in the final entry and `found.after` is only ever the remainder
      // of the utterance it was spoken in - the rest of that sentence, never a
      // later thought. Holding that remainder back would split one spoken
      // instruction across two prompts.
      const spoken = [found.before, found.after]
        .map((part) => part.trim())
        .filter(Boolean)
        .join(" ");
